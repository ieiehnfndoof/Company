...
v263(v300, "Window")
local u305
v289(v300, "ResetWin", "Reset position & size", "window and navigation back to the middle at default size", "Reset", function()
    ...
    if EGOYNavRoot then
        EGOYNavRoot.Position = UDim2.fromScale(0.13, 0.5)
    end
...
-- EGOY navigation replaces the legacy left tab rail. The legacy rail remains in the
-- source for compatibility with the existing page model but is hidden from the UI.
v151.Visible = false
v151.Active = false
...
local EGOYNavTweenTime = 0.28

local function EGOYNavTween(obj, props, duration, style, direction)
    if not obj then return end
    local tween = TweenService:Create(obj, TweenInfo.new(duration or EGOYNavTweenTime, style or Enum.EasingStyle.Quint, direction or Enum.EasingDirection.Out), props)
    tween:Play()
    return tween
end
...
function u70(p146)
    if p146 then
        ...
        if EGOYNavRoot then EGOYNavRoot.Visible = true end
        if v306 then v306.Visible = true end
        -- Soft menu entrance: scale up from a slightly reduced size.
        v108.Scale = math.max(0.94, (tonumber(t9.UIScale) or 100) / 100 * 0.94)
        EGOYNavTween(v108, {Scale = (tonumber(t9.UIScale) or 100) / 100}, 0.34, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
        for i, item in ipairs(EGOYNavItems) do
            if item and item.button then
                ...
                item.button.Size = UDim2.fromOffset(1, 1)
        ...
        -- Soft menu exit: shrink first, then hide it, leaving only the logo.
        EGOYNavExpanded = false
        for i = #EGOYNavItems, 1, -1 do
            local item = EGOYNavItems[i]
            if item and item.button then
                task.delay((#EGOYNavItems - i) * 0.055, function()
        ...
        EGOYNavTween(v108, {Scale = math.max(0.86, (tonumber(t9.UIScale) or 100) / 100 * 0.92)}, 0.24, Enum.EasingStyle.Quint, Enum.EasingDirection.In)
...
-- Build the radial navigation after the page definitions exist, using the same
-- decoded logo asset already embedded in the original source.
EGOYNavRoot = v82("Frame", {
    Name = "EGOYNavigation",
    AnchorPoint = Vector2.new(0.5, 0.5),
...
local navCenterPosition = UDim2.fromScale(0.5, 0.5)
v306.Parent = EGOYNavRoot
v306.AnchorPoint = Vector2.new(0.5, 0.5)
v306.Position = navCenterPosition
v306.Visible = true
v306.Size = UDim2.fromOffset(not u42 and 42 or 50, not u42 and 42 or 50)
v306.ZIndex = 75
...
for i, entry in ipairs(navNames) do
    ...
    local button = v82("TextButton", {
        Name = "Nav_" .. entry.name:gsub("%s+", ""),
        AnchorPoint = Vector2.new(0.5, 0.5),
        Position = open,
    ...
    button.MouseEnter:Connect(function()
        EGOYNavTween(button, {Size = UDim2.fromOffset(navSize + 6, navSize + 6), BackgroundTransparency = 0}, 0.16)
        if icon then EGOYNavTween(icon, {Size = UDim2.fromOffset(22, 22)}, 0.16) end
    end)
    button.MouseLeave:Connect(function()
        EGOYNavTween(button, {Size = UDim2.fromOffset(navSize, navSize), BackgroundTransparency = 0.08}, 0.18)
        if icon then EGOYNavTween(icon, {Size = UDim2.fromOffset(19, 19)}, 0.18) end
    end)
    button.MouseButton1Click:Connect(function()
        u68(entry.name)
    end)
    EGOYNavItems[i] = {button=button, openPosition=open, closedPosition=closed, size=navSize}
end

-- Drag the complete navigation as one unit. The center logo remains draggable
-- while collapsed because it is still the same navigation root.
EGOYNavRoot.InputBegan:Connect(function(input)
...
v73(UserInputService.InputChanged:Connect(function(input)
    if not EGOYNavDragging then return end
    local typ = input.UserInputType
    if typ ~= Enum.UserInputType.MouseMovement and typ ~= Enum.UserInputType.Touch then return end
    local delta = input.Position - EGOYNavDragStart
    EGOYNavRoot.Position = UDim2.new(
        EGOYNavStartPos.X.Scale, EGOYNavStartPos.X.Offset + delta.X,
        EGOYNavStartPos.Y.Scale, EGOYNavStartPos.Y.Offset + delta.Y
    )
end))
...
function u44()
    ...
    n3 = 0
    v151.Size = UDim2.new(0, 0, 1, 0)
    v194.Position = UDim2.fromOffset(0, 2)
    v194.Size = UDim2.new(1, 0, 1, -2)
