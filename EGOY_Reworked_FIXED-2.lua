        ...
        do
            ...
            t2 = {
				Dark = {
					...
					rail = Color3.fromRGB(9, 15, 27),
					card = Color3.fromRGB(13, 21, 36),
					lift = Color3.fromRGB(18, 29, 48),
					fill = Color3.fromRGB(21, 35, 58),
					line = Color3.fromRGB(30, 67, 105),
					text = Color3.fromRGB(235, 247, 255),
					dim = Color3.fromRGB(145, 174, 198),
					mute = Color3.fromRGB(91, 121, 148),
					accent = Color3.fromRGB(45, 166, 255),
					accentDeep = Color3.fromRGB(18, 76, 125),
					accentHover = Color3.fromRGB(86, 190, 255),
					ink = Color3.fromRGB(4, 10, 18),
					ok = Color3.fromRGB(104, 205, 153),
					EGOY = Color3.fromRGB(235, 247, 255)
				},
				Light = {
...
local v294 = v259("About", "EGOY SCRIPT for Steal an Egg")
...
v263(v294, "Credits")
v293(v294, "Created by", v291(t1.Author) and t1.Author or t1.Title)
if v291(t1.Credits) then
    v293(v294, "Credits", t1.Credits)
end
...
-- EGOY radial navigation -------------------------------------------------
-- The original K-logo is already produced by v87/v78.  Keep that asset and
-- use it as the center button rather than replacing it with a new image.
local EGOYTweenService = game:GetService("TweenService")
local EGOYNavRoot
local EGOYNavItems = {}
local EGOYNavExpanded = true
local EGOYNavDragging = false
local EGOYNavDragStart
local EGOYNavStartPos
local EGOYNavDragMoved = false
local EGOYNavTweenTime = 0.30

local function EGOYNavTween(obj, props, duration, style, direction)
    if not obj then
        return
    end

    local ok, tween = pcall(function()
        return EGOYTweenService:Create(obj, TweenInfo.new(
            duration or EGOYNavTweenTime,
...
local function EGOYNavSetMenuVisible(visible)
    u308 = not not visible
    v113.Visible = u308
...
function u70(p146)
    if p146 then
        EGOYNavExpanded = true
        EGOYNavSetMenuVisible(true)

        if EGOYNavRoot then
            EGOYNavRoot.Visible = true
        end
        v306.Visible = true

        -- Main window: soft scale + fade-like slide through the existing UI scale.
        local targetScale = (tonumber(t9.UIScale) or 100) / 100
        v108.Scale = targetScale * 0.965
        EGOYNavTween(v108, {Scale = targetScale}, 0.36, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)

        -- Radial wedges rise out from the center one by one.
        for i, item in ipairs(EGOYNavItems) do
            if item and item.button then
                item.button.Visible = true
                item.button.Position = item.closedPosition
                item.button.Size = UDim2.fromOffset(4, 4)
                item.button.BackgroundTransparency = 1

                task.delay((i - 1) * 0.075, function()
                    if EGOYNavExpanded and item.button then
                        EGOYNavTween(item.button, {
                            Position = item.openPosition,
                            Size = UDim2.fromOffset(item.size, item.size),
        ...
        EGOYNavExpanded = false

        -- Retract in reverse order, deliberately slower so the menu does not snap.
        for i = #EGOYNavItems, 1, -1 do
            local item = EGOYNavItems[i]
            if item and item.button then
                task.delay((#EGOYNavItems - i) * 0.075, function()
                    if not EGOYNavExpanded and item.button then
                        EGOYNavTween(item.button, {
                            Position = item.closedPosition,
                            Size = UDim2.fromOffset(4, 4),
                            BackgroundTransparency = 1
                        }, 0.30, Enum.EasingStyle.Quint, Enum.EasingDirection.In)
                    end
                end)
            end
        end
        ...
        EGOYNavTween(v108, {
            Scale = math.max(0.92, ((tonumber(t9.UIScale) or 100) / 100) * 0.965)
        }, 0.30, Enum.EasingStyle.Quint, Enum.EasingDirection.In)

        task.delay(0.34 + (#EGOYNavItems * 0.075), function()
            if not EGOYNavExpanded then
                EGOYNavSetMenuVisible(false)
            end
        end)
    end
end

-- Keep the existing window close button working.
v217.MouseButton1Click:Connect(function()
    u70(false)
end)

EGOYNavRoot = Instance.new("Frame")
EGOYNavRoot.Name = "EGOYNavigation"
EGOYNavRoot.AnchorPoint = Vector2.new(0.5, 0.5)
EGOYNavRoot.Position = UDim2.fromScale(0.13, 0.50)
EGOYNavRoot.Size = UDim2.fromOffset(250, 250)
EGOYNavRoot.BackgroundTransparency = 1
EGOYNavRoot.BorderSizePixel = 0
EGOYNavRoot.Active = true
EGOYNavRoot.ZIndex = 70
EGOYNavRoot.Parent = v98

-- Center is the original Orb + original K logo created by v87.
v306.Parent = EGOYNavRoot
v306.AnchorPoint = Vector2.new(0.5, 0.5)
v306.Position = UDim2.fromScale(0.5, 0.5)
v306.Size = UDim2.fromOffset(46, 46)
v306.Visible = true
v306.ZIndex = 75

local navStroke = Instance.new("UIStroke")
navStroke.Color = t3.accent
navStroke.Transparency = 0.28
navStroke.Thickness = 1.2
navStroke.Parent = v306

local navGlow = Instance.new("UIStroke")
navGlow.Color = t3.accent
navGlow.Transparency = 0.82
navGlow.Thickness = 4
navGlow.Parent = v306

local navCenterScale = Instance.new("UIScale")
navCenterScale.Scale = 1
navCenterScale.Parent = v306

local navNames = {
    {name = "About", icon = "info"},
    {name = "Auto Steal", icon = "egg"},
    {name = "Plot", icon = "grid"},
    {name = "Serverhop", icon = "rocket"},
    {name = "Misc", icon = "layers"},
    {name = "Webhook", icon = "out"},
...
local function EGOYNavBeginDrag(input)
    local typ = input.UserInputType
    if typ == Enum.UserInputType.MouseButton1 or typ == Enum.UserInputType.Touch then
        EGOYNavDragging = true
        EGOYNavDragMoved = false
        EGOYNavDragStart = input.Position
        EGOYNavStartPos = EGOYNavRoot.Position
    end
end

EGOYNavRoot.InputBegan:Connect(EGOYNavBeginDrag)

for i, entry in ipairs(navNames) do
    local angle = math.rad(-90 + (i - 1) * (360 / #navNames))
    local x = math.cos(angle) * navRadius
    local y = math.sin(angle) * navRadius
    local closed = UDim2.new(0.5, 0, 0.5, 0)
    local open = UDim2.new(0.5, x, 0.5, y)

    local button = Instance.new("TextButton")
    button.Name = "Nav_" .. entry.name:gsub("%s+", "")
    button.AnchorPoint = Vector2.new(0.5, 0.5)
    button.Position = open
    button.Size = UDim2.fromOffset(navSize, navSize)
    button.BackgroundColor3 = t3.card
    ...
    local icon = v86(button, entry.icon, t3.mute, 19)
    if icon then
    ...
    button.MouseEnter:Connect(function()
        EGOYNavTween(button, {
            Size = UDim2.fromOffset(navSize + 5, navSize + 5),
            BackgroundTransparency = 0
        }, 0.16)
    end)
    ...
    button.MouseLeave:Connect(function()
        EGOYNavTween(button, {
            Size = UDim2.fromOffset(navSize, navSize),
            BackgroundTransparency = 0.10
        }, 0.18)
    end)

    button.InputBegan:Connect(EGOYNavBeginDrag)

    button.MouseButton1Click:Connect(function()
        -- The normal click still changes the existing page model.
        u68(entry.name)
    end)
...
-- The logo is also the collapse/restore control.
v306.InputBegan:Connect(EGOYNavBeginDrag)
v306.MouseButton1Click:Connect(function()
    if EGOYNavDragging and EGOYNavDragMoved then
        return
    end

    if EGOYNavExpanded then
        u70(false)
    else
        u70(true)
    end
...
v73(UserInputService.InputChanged:Connect(function(input)
    if not EGOYNavDragging or not EGOYNavDragStart or not EGOYNavStartPos then
        return
    end

    local typ = input.UserInputType
    if typ ~= Enum.UserInputType.MouseMovement and typ ~= Enum.UserInputType.Touch then
        return
    end

    local delta = input.Position - EGOYNavDragStart
    if math.abs(delta.X) > 3 or math.abs(delta.Y) > 3 then
        EGOYNavDragMoved = true
    end

    EGOYNavRoot.Position = UDim2.new(
        EGOYNavStartPos.X.Scale,
        EGOYNavStartPos.X.Offset + delta.X,
        EGOYNavStartPos.Y.Scale,
        EGOYNavStartPos.Y.Offset + delta.Y
    )
end))
...
-- Hide only the old visual rail. The existing page model and callbacks remain intact.
v151.Visible = false
v151.Active = false
...
(function()
    ...
    local u1195 = (function()
        ...
        local function v1811(p272)
            ...
            t184.text = v3018 == "" and "EGOY SCRIPT" or "EGOY SCRIPT · " .. v3018
            t183.footer = t184
    ...
    print("[UI] EGOY SCRIPT ready · RightShift toggles · Dark / Light in Settings")
